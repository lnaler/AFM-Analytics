 ...
 *
 */
public class GeneticIndentFinder{
	 ...
	 */
	public GeneticIndentFinder(int iterations, CurveData testData)
	{
		comparePoints = testData.getDogLegPoints();
		//comparePoints = points;
		numGenerations = iterations;
		this.bounds = testData.getXBounds();
		data = testData;
		//shiftPoints();
	}
	...
	public boolean run()
	{
		...
		double ga_input= 0;   //initial chromosome (1 gene) Slope 
		double limits[] = new double[3];
		Random rand = new Random();
		limits[0]= bounds[0];    //lower limits for each gene 
		limits[1]= bounds[1];    //upper limits for each gene ~1113kPa
		...
		for(int i = 0; i < numChromosomes;i++)
		{
			...
			//3 decimal places
			long key = Math.round(chromResult.get(i)*1000000.0);
...
//			
		...
		while(generation < numGenerations)
		{
			...
			{
				long key = Math.round(chromResult.get(i)*1000000.0);
				if(Double.isInfinite(chromResult.get(i)))
				{
			...
			System.out.println("Computing generation " + generation + " of " + numGenerations + ". Optimum result: " + (chromOld.firstKey()/1000000.0));  
			generation++;
		}
		...
		System.out.println("Optimum Indent: " + optimumChrom + " Error: " + optimumResult);
		System.out.println("Spacer Line");
		//System.out.println("Least Squares: " + getLS());
		return true;
	}
	
	private double getSlope(double test_indent)
	{
		data.setImpactZ(test_indent);
		data.initMatrix();
		data.calcMatrix();
		double slope = data.fitAndGetSlope();
		comparePoints = data.getDogLegPoints();
		return slope;
	}
	 ...
	 */
	private double getError(double test_indent)
	{
		double slope = getSlope(test_indent);
		double error = 0;
		double n = comparePoints.size();
		for(int i = 0;i < n; i++)
		{
			//double x = Math.exp(comparePoints.get(i).x);
			//double actY = Math.exp(comparePoints.get(i).y);
		...
		long test = Math.round(error*1000000.0);
		if(test == 0)
		{
			System.out.println("Hold up!");
		}
